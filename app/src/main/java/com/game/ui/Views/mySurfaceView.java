   ...
   /* public mySurfaceView(Context context) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
       // surfaceView.setZOrderOnTop(true);
        ...
        // 修复Android 11+触摸偏移问题
        // 使用getX/Y()获取相对于视图的坐标，而不是getRawX/Y()
        // 这样可以避免系统栏/刘海屏导致的坐标偏移
        ...
        try {
            bl = GLES3JNILib.MotionEventClick(event.getAction(), touchX, touchY);
        } catch (Exception e) {
            Log.e("NDK-java", "MotionEvent error: " + e.getMessage());
            bl = false;
        }
